...
const EMPTY_CREATE = { ho_va_ten: '', so_dien_thoai: '', email: '', cccd: '', password: '', re_password: '', dia_chi: '', id_quan_huyen: '', is_active: '1', is_block: '0' };
...
const ShipperFormFields = ({ form, onChange, inclPass = false, tinhList, qhList, idTinh, onTinhChange, onQHLoad }) => (
  <div className="grid sm:grid-cols-2 gap-3">
    ...
    <div><label className={LABEL}>Tình trạng</label><select className={INPUT} value={form.is_block ?? '0'} onChange={e => onChange({ ...form, is_block: e.target.value })}><option value="0">Hoạt động</option><option value="1">Đã chặn</option></select></div>
  </div>
);
        ...
        <div className="flex gap-3">
          ...
          <ExcelButton color="green" disabled={list.length === 0} onClick={() => exportToExcel(
            ...
            [
              ...
              { label: 'Tổng chuyến', key: 'tong_chuyen', width: 12 },
              { label: 'Hạng', key: 'hang_shipper', width: 15 },
              ...
              { label: 'Đang chạy xe', key: 'is_open', width: 14, format: v => v == 1 ? 'Có' : 'Không' },
              { label: 'Tình trạng', key: 'is_block', width: 14, format: v => v == 1 ? 'Đã chặn' : 'Hoạt động' },
                  ...
                  <tr key={s.id || i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <span className="font-bold text-gray-700">{s.tong_chuyen || 0} chuyến</span>
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${s.hang_shipper === 'Kim Cương' ? 'bg-purple-100 text-purple-700' : s.hang_shipper === 'Vàng' ? 'bg-yellow-100 text-yellow-700' : s.hang_shipper === 'Bạc' ? 'bg-gray-200 text-gray-700' : 'bg-orange-100 text-orange-700'}`}>
                          {s.hang_shipper || 'Đồng'}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold w-full ${s.is_open == 1 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        {s.is_open == 1 ? '🟢 Có' : '⚪ Không'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => changeStatus(s)} className={`px-3 py-1 rounded-lg text-xs font-bold w-full transition-colors ${s.is_block == 1 ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
                        {s.is_block == 1 ? 'Đã chặn' : 'Hoạt động'}</button>
                    </td>
                    <td className="px-4 py-3 text-center">
