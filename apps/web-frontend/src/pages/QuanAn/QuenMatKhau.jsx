...
export default function QuanAnQuenMatKhau() {
  ...
  const guiMa = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/quan-an/gui-ma-quen-mat-khau', { email });
  ...
  const datLaiMatKhau = async e => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/quan-an/quen-mat-khau', { email, ma_otp: otp, new_password: newPassword });
      if (res.data.status == 1) {
        toast.success(res.data.message);
        setTimeout(() => navigate('/quan-an/dang-nhap'), 1500);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center shadow-2xl"
      style={{ background: 'linear-gradient(135deg, #1e3a8a, #1e40af, #1e3a8a)' }}>
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        <div className="bg-white/10 backdrop-blur-2xl rounded-3xl p-10 shadow-2xl border border-white/20 animate-fade-up">
          ...
          <div className="text-center mb-6">
            ...
            <h3 className="text-xl font-bold text-white/80 mt-2">Quên Mật Khẩu Quán Ăn</h3>
          ...
          <div className="flex items-center justify-center mb-8 gap-0">
            {[1, 2].map((s, si) => (
              <div key={s} className="flex items-center">
                {si > 0 && (
                  <div className={`w-12 h-0.5 mx-2 transition-all duration-500 ${step > s - 1 ? 'bg-blue-400' : 'bg-white/10'}`} />
                )}
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-400 ${step > s ? 'bg-green-500 text-white' : step === s ? 'text-white' : 'bg-white/10 text-white/30'}`}
                    style={step === s ? { backgroundColor: '#3b82f6', boxShadow: '0 4px 12px rgba(59,130,246,0.3)' } : {}}>
            ...
            <form onSubmit={guiMa} className="space-y-5">
              <div>
                <label className="block text-sm font-semibold text-white/80 mb-2">
                  <i className="fa-solid fa-envelope mr-2 text-blue-400" />Email đăng ký
                </label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="email@quanan.com" required 
                  className="w-full px-4 py-3.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder-white/30 focus:outline-none focus:border-blue-400 transition-all font-medium" />
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-70 bg-blue-500 text-white"
              >
            ...
            <form onSubmit={datLaiMatKhau} className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-white/80 mb-3">
                  <i className="fa-solid fa-shield-halved mr-2 text-blue-400" />Mã xác nhận (6 số)
                </label>
                <div className="flex gap-2 justify-center" onPaste={onOtpPaste}>
                  {otpDigits.map((d, i) => (
                    <input key={i} ref={el => otpRefs.current[i] = el} type="text" inputMode="numeric"
                      ...
                      className="w-12 h-14 text-center text-2xl font-bold text-blue-400 rounded-xl outline-none transition-all duration-200 border-2 border-white/10 bg-white/5 focus:border-blue-400 focus:bg-white/10"
                    />
                  ))}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={newPassword}
                      ...
                      className="w-full px-4 py-3.5 rounded-xl bg-white/10 border border-white/20 text-white focus:outline-none focus:border-blue-400 transition-all" />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showConfirm ? 'text' : 'password'} value={confirmPassword}
                      ...
                      className={`w-full px-4 py-3.5 rounded-xl bg-white/10 border border-white/20 text-white focus:outline-none focus:border-blue-400 transition-all ${confirmPassword && newPassword !== confirmPassword ? 'border-red-400' : ''}`} />
          ...
          <div className="text-center mt-6">
            <Link to="/quan-an/dang-nhap"
              className="text-blue-400 hover:text-blue-300 text-sm font-medium flex items-center justify-center gap-1 transition-all hover:-translate-x-1">
