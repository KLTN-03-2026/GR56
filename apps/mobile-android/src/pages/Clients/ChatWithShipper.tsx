    ...
    <>
      ...
      <Animated.View style={{ transform: [{ scale }], opacity }}>
        <View
          style={[
            s.msgRow,
            isSent ? s.msgRowSent : s.msgRowReceived,
          ]}
        >
          {!isSent && (
          ...
          <View
            style={[
              s.bubbleWrapper,
              isSent ? { alignItems: "flex-end" } : { alignItems: "flex-start" },
            ]}
          >
            <View
              style={[
            ...
            <View
              style={[
                s.timeRow,
                isSent ? { justifyContent: "flex-end" } : { justifyContent: "flex-start" },
              ]}
            >
...
const ChatWithShipper = ({ navigation, route }: any) => {
  const {
    id_don_hang,
    name = "Shipper",
    avatar,
  ...
  const loaiToiRef = useRef<string>("khach_hang");
  ...
  const loadMessages = useCallback(
    async () => {
      ...
      try {
        const res = await apiClient.get(`/khach-hang/chat/${id_don_hang}/tin-nhan`);
        if (res.data?.status && Array.isArray(res.data.data)) {
          ...
          if (newMsgs.length > 0) {
            ...
            // Phát hiện tin nhắn mới từ shipper
        ...
        setTimeout(() => {
          apiClient.get(`/khach-hang/chat/${id_don_hang}/tin-nhan`)
            .then(res => {
              if (res.data?.status && Array.isArray(res.data.data)) {
                const loaiToi: string = res.data.loai_toi || loaiToiRef.current;
                loaiToiRef.current = loaiToi;
  ...
  useEffect(() => {
    ...
    return () => {
      if (echoInstance) {
        // Chỉ leave channel, KHÔNG disconnect() để khách hàng vẫn nhận WS event
  ...
  const animateSend = () => {
    Animated.sequence([
      Animated.spring(sendScale, { toValue: 0.8, useNativeDriver: true, tension: 250 }),
      Animated.spring(sendScale, { toValue: 1, useNativeDriver: true, tension: 120 }),
    ]).start();
  };
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await apiClient.post("/khach-hang/chat/gui", {
        id_don_hang,
        noi_dung: text,
      });
  ...
  const quickReplies = [
    { emoji: "📍", label: "Đang ở đâu?", text: "Bạn đang ở đâu?" },
    { emoji: "🏠", label: "Tôi ở nhà!", text: "Tôi ở nhà rồi!" },
    { emoji: "⚡", label: "Giao nhanh nhé!", text: "Giao nhanh giúp mình nhé!" },
    { emoji: "👍", label: "Cảm ơn!", text: "OK, cảm ơn!" },
  ];
      ...
      >
          ...
          <View style={s.headerCenter}>
            ...
            <View style={s.headerMeta}>
              ...
              <View style={s.headerSubRow}>
                ...
                <Text style={s.headerSub}>Shipper · Đang hoạt động</Text>
              </View>
            </View>
          </View>
        ...
        {ma_don_hang && (
          <View style={s.orderBanner}>
            <View style={s.orderIconCircle}>
              <Ionicons name="bicycle" size={17} color={PRIMARY} />
            </View>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={s.intro}>
              ...
              <Text style={s.introRole}>Shipper · FoodBee</Text>
              ...
              <View style={s.statsCard}>
                ...
                <View style={s.statItem}>
                  <Text style={s.statValue}>1.2k</Text>
        ...
        <View style={[s.inputBar, inputFocused && s.inputBarFocused]}>
          ...
          <View style={[s.inputWrap, inputFocused && s.inputWrapFocused]}>
            <TextInput
              style={s.input}
              placeholder="Nhắn tin với shipper..."
...
const s = StyleSheet.create({
  ...
  verifiedText: { fontSize: wp("2.7%"), color: PRIMARY, fontWeight: "700" },
  introName: {
    fontSize: wp("5%"),
    fontWeight: "800",
    color: "#0F172A",
  ...
  statLabel: { fontSize: wp("2.7%"), color: "#94A3B8", fontWeight: "500", marginTop: 2 },
