  ...
  return (
    <Animated.View style={[styles.detailPanel, { transform: [{ translateY: slideAnim }] }]}>
      ...
      <DeliveryMapModal
        ...
        destAddress={order.tinh_trang <= 2 ? order.dia_chi_quan : deliveryAddr}
        destName={order.tinh_trang <= 2 ? order.ten_quan_an : order.ten_nguoi_nhan}
        destCoord={
          order.tinh_trang === 3 && order.toa_do_x && order.toa_do_y
...
const ShipperOrders = ({ navigation }: any) => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      // Tab Nhận đơn — tiên_trang=0, chưa có shipper
      if (r1.data?.list_don_hang_co_the_nhan) {
        setAvailableOrders(
          r1.data.list_don_hang_co_the_nhan.map((o: any): Order => ({
            ...o,
            tinh_trang: 0,
            activeTab: "nhan_don",
          }))
